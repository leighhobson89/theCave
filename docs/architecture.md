...
- Cards load `[progressTimeLineEventId].png` from `assets/progressEvidenceImages/`, falling back to a placeholder card carrying the id when the file does not exist yet. A card is the artwork alone — the id is its `aria-label` and data attribute, not print under it — so the photograph gets the whole card.
- Each unlocked frame on the board carries a one-line **note field** above its photograph plus a cross that clears it; the note is saved, is played back as the photograph's hover tooltip, and is removed and deleted when the frame locks. See [progress-timeline-event-system.md](progress-timeline-event-system.md).
