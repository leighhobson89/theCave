...
| `camera-reset-on-scene-entry.spec.js` | Every route that arrives at a gameplay scene — the noticeboard toggle (both directions), Resume Game, and Load Game — resets the camera to minimum zoom and that scene's own anchor (noticeboard bottom, desk centre), proven against a camera deliberately moved away from default first |
